  ...
  stepSize(stepSize), t(0), e(0), policy(epsilon) /* EpsilonGreedyPolicy(epsilon) */
{
    ...
    {
        reward_correlation_table[a] = new double[nActions];
        tally_correlation_table = new double[nActions];
    }
...
double * expectationFromCorrelations(double * actionExpt, double ** rTable,
                                     double **cTable, double gamma,
                                     int nActions, static vector<int> history_A,
                                     int t)
// sort it into Agent:: or policy?
{
    ...
    for (int a = 0; a<nActions; ++a)
    {
        ...
        for (int h = t-1; h>=0; --h)
        {
            actionExpt[a] += pow * rTable[a][history_A[h]]/cTable[a][history_A[h]];
            pow *= gamma;
        }
        total+= actionExpt[a];
    }
...
void updateCorrelationMatrices(double lastReward, double ** rTable,
                               double **cTable, double gamma,
                               int nActions, static vector<int> history_A,
                               int t)
    //gamma as a pointer, so it updates as well...
{
    ...
    for (int h = t-2; h>0; h--)
    {
        ...
        {
            rTable[a][history_A[h]] += powpow*pow*lastReward; //discount: gamma^(most distant action)
            cTable[a][history_A[h]] += powpow*pow;
            powpow*= gamma;
        }
    }
